...
static std::string
makeSubCategoriesString(const std::vector<std::string> &subCategories) {
  if (subCategories.empty())
    return {};
  std::string r;
  r.reserve(64);
  for (const auto &c : subCategories)
    r += "|" + c;
  r += "|";
  return r;
}
...
Space::getComponent(const std::string &cidString) {
  if (const auto cid{VST3::UID::fromString(cidString)})
    return getModule(cidString)
        ->getFactory()
        .createInstance<Steinberg::Vst::IComponent>(cid.value());
  throw std::runtime_error("bad UID string: " + cidString);
}

Steinberg::IPtr<Steinberg::Vst::IEditController>
Space::getEditor(const VST3::UID &editorID) {
  return getModule(editorID.toString())
      ->getFactory()
      .createInstance<Steinberg::Vst::IEditController>(editorID);
}
...
bool Space::fromCache(const std::filesystem::path &cachePath,
  ...
  if (line != cacheHeader)
    throw std::runtime_error("bad merc::Space cache");
  while (std::getline(cache, line)) {
    ...
    if (!std::filesystem::is_directory(modulePath) ||
        modulePath.extension() != ".vst3" || !isInDirs(modulePath, dirs)) {
      entries.clear();
      return false;
    }
    long long lmt;
...
void Space::addClassesOrRecurse(
    const std::filesystem::path &dir,
    const std::unordered_set<std::string> &modulesToIgnore) {
  if (dir.extension() == ".vst3" && !modulesToIgnore.contains(dir.string()))
    addClasses(dir);
  else
...
void Space::addClasses(const std::filesystem::path &modulePath) {
  if (const auto infoPath{
          VST3::Hosting::Module::getModuleInfoPath(modulePath.string())}) {
    if (const auto info{Steinberg::ModuleInfoLib::parseJson(
            Steinberg::readFile(infoPath.value()), nullptr)}) {
      for (const auto &c : info.value().classes)
        entries.insert(
            {c.cid,
             {c.name, c.category, makeSubCategoriesString(c.subCategories),
              modulePath.string()}});
      return;
    }
  }
...
void Space::writeCache(const std::filesystem::path &cachePath) const {
  ...
  for (const auto &[cid, entry] : entries)
    out << cid << "," << entry.name << "," << entry.category << ","
        << entry.subCategories << "," << entry.modulePath << ","
        << getLmt(entry.modulePath) << "\n";
}

long long Space::getLmt(const std::filesystem::path &path) {
  const auto lmtDuration{
      std::filesystem::last_write_time(path).time_since_epoch()};
  return std::chrono::duration_cast<std::chrono::milliseconds>(lmtDuration)
      .count();
}

bool Space::isInDirs(const std::filesystem::path &path,
                     const std::set<std::filesystem::path> &dirs) {
  for (const auto &dir : dirs)
    if (std::mismatch(path.begin(), path.end(), dir.begin()).second ==
        dir.end())
      return true;
  return false;
}
