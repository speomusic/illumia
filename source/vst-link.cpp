...
template <>
Samples
createResource<structure::Link>(Steinberg::Vst::SpeakerArrangement arrangement,
                                const Merc &merc) {
  return Samples{2, global::getConfig().audio.batchSize};
}

template <>
Steinberg::Vst::SpeakerArrangement getMainOutArrangement<structure::Link>() {
  return Steinberg::Vst::SpeakerArr::kStereo;
}

VstElementImplementation<structure::Link>::VstElementImplementation(
    ElementImplementation<structure::Link, structure::VstRunner> &ve)
    : vstElement{ve} {}

void VstElementImplementation<structure::Link>::setupForMerc() {
  if (vstElement.canReceiveEvents()) {
    vstElement.inEvents = std::make_unique<Steinberg::Vst::EventList>(
        global::getConfig().audio.eventBufferSize);
    vstElement.eventGate = std::make_unique<Gate<TimedEvent>>(
        global::getConfig().audio.eventBufferSize);
  }
  Steinberg::Vst::ProcessSetup setup{
      Steinberg::Vst::kRealtime, Steinberg::Vst::kSample32,
      static_cast<int>(global::getConfig().audio.batchSize),
      static_cast<Steinberg::Vst::SampleRate>(
          global::getConfig().audio.sampleRate)};
  CHECK_TRESULT(vstElement.processor->setupProcessing(setup));
}

void VstElementImplementation<structure::Link>::setBusArrangement(
    std::vector<Steinberg::Vst::SpeakerArrangement> &inArrangements,
    std::vector<Steinberg::Vst::SpeakerArrangement> &outArrangements) {
  CHECK_TRESULT(vstElement.processor->setBusArrangements(
      inArrangements.data(), (int)inArrangements.size(), outArrangements.data(),
      (int)outArrangements.size()));
}
...
void VstElementImplementation<structure::Link>::setupProcessData() {
  ...
  for (const auto &out : vstElement.element.media.outs)
    totalNumChannels +=
        std::visit(OutChannelCount{vstElement.element.merc}, out);
  ...
  for (int i{0}; i < vstElement.element.media.ins.size(); ++i) {
    inAudioBusBuffers[i].numChannels =
        std::visit(InChannelCount{vstElement.element.merc},
                   vstElement.element.media.ins[i]);
  ...
  for (int i{0}; i < vstElement.element.media.outs.size(); ++i) {
    outAudioBusBuffers[i].numChannels =
        std::visit(OutChannelCount{vstElement.element.merc},
                   vstElement.element.media.outs[i]);
...
void VstElementImplementation<structure::Link>::run(uint64_t iterationIndex) {
  for (int i{0}; i < vstElement.element.media.ins.size(); ++i)
    std::visit(InSetChannelData{vstElement.element.merc, inAudioBusBuffers[i],
                                iterationIndex % 2},
               vstElement.element.media.ins[i]);
  for (int i{0}; i < vstElement.element.media.outs.size(); ++i)
    std::visit(OutSetChannelData{vstElement.element.merc, outAudioBusBuffers[i],
                                 (iterationIndex + 1) % 2},
               vstElement.element.media.outs[i]);
