...
ElementImplementation<T, structure::VstRunner>::ElementImplementation(
    Element<T> &e, structure::VstRunner runner)
    : element{e}, cid{runner.cid}, component{element.merc.makeComponent(cid)},
      processor{Steinberg::FUnknownPtr<typename VstElementTraits<T>::Processor>{
          component}},
      impl{*this} {
  impl.setupForMerc();
...
void ElementImplementation<T, structure::VstRunner>::activate() {
  for (int i{0}; i < component->getBusCount(Steinberg::Vst::kEvent,
                                            Steinberg::Vst::kInput);
       ++i)
    CHECK_TRESULT(component->activateBus(Steinberg::Vst::kEvent,
                                         Steinberg::Vst::kInput, i, true));

  for (int i{0}; i < element.routing.ins.size(); ++i)
    CHECK_TRESULT(component->activateBus(ElementTraits<T>::defaultMediaType,
                                         Steinberg::Vst::kInput, i, true));
...
template <typename T>
void ElementImplementation<T, structure::VstRunner>::pushEvent(
    TimedEvent event) {
  eventGate->push(event);
}

template <typename T>
void ElementImplementation<T, structure::VstRunner>::pumpInputEvents(
    size_t startSampleIndex, size_t samplesPerRun) {
  while (auto *timedEvent{eventGate->peak()}) {
    ...
    if (timedEvent->sample > startSampleIndex)
      timedEvent->event.sampleOffset =
          Steinberg::int32(timedEvent->sample - startSampleIndex);
...
template <typename T>
void ElementImplementation<T, structure::VstRunner>::run(
    const TimeKeeper<Element<T>> &timeKeeper) {
  const auto iterationIndex{timeKeeper.getCurrentIterationIndex()};
  const auto samplesPerIteration{
      TimeKeeper<Element<T>>::getSamplesPerIteration()};
  if (canReceiveEvents())
    pumpInputEvents(size_t(iterationIndex * samplesPerIteration),
...
template <typename T>
void ElementImplementation<T,
                           structure::VstRunner>::onRoutingAndMediaChanged() {
  deactivate();
  impl.setupProcessData();
  activate();
}
...
bool ElementImplementation<T, structure::VstRunner>::canReceiveEvents() const {
  return component->getBusCount(Steinberg::Vst::kEvent,
