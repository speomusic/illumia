...
void FaustWrapperBase::addVerticalSlider(const char *name, float *ptr,
...
NoteCV::NoteCV(int busIndex, int channelMask)
    : busIndex{busIndex}, channelMask{channelMask} {}
...
void NoteCV::get(Steinberg::Vst::IEventList *eventList, int channel) {
  ...
  for (int i{0}; i < gain.size(); ++i) {
    if (getPreviousGain(i) == 0.0f && gain[i] != 0.0f) {
      Steinberg::Vst::Event e{busIndex, i, 0.0, 0,
                              Steinberg::Vst::Event::kNoteOnEvent};
      e.noteOn = Steinberg::Vst::NoteOnEvent{(Steinberg::int16)channel,
      ...
      e.noteOff = Steinberg::Vst::NoteOffEvent{
          (Steinberg::int16)channel, (Steinberg::int16)note[i], 0.0f, -1, 0.0f};
      CHECK_TRESULT(eventList->addEvent(e));
    }
  }
