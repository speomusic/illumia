...
namespace merc::vst
{
    ...
    struct VoicePool
    {
        VoicePool(size_t numVoices)
            : voices(numVoices)
        {}
        void runVoices(Steinberg::Vst::IEventList& eventList, typename VoiceImpl::Context* context)
        {
            for (int eventIndex{ 0 }; eventIndex < eventList.getEventCount(); ++eventIndex)
            {
                ...
                switch (event.type)
                {
                    case Steinberg::Vst::Event::kNoteOnEvent:
                        getNonPlayingVoice(context).asPlaying(event.noteOn).impl.start(context, event);
                        break;
    ...
    private:
        ...
        {
            for (auto& voice : voices) if (!voice.playing) return voice;
            stopVoice(voices.front(), context);
            return voices.front();
        }

        void stopVoice(typename VoiceImpl::Context* context, const Steinberg::Vst::Event& event)
        {
        ...
        {
            Steinberg::Vst::NoteOffEvent event
            {
                .channel = voice.noteOnEvent.channel,
            ...
            voice.playing = false;
            voice.impl.stop(context, { .type = Steinberg::Vst::Event::kNoteOffEvent, .noteOff = event });
        }

        void stopVoice(Voice<VoiceImpl>& voice, typename VoiceImpl::Context* context, const Steinberg::Vst::Event& event)
        {
            voice.playing = false;
            voice.impl.stop(context, event);
        }

        Voice<VoiceImpl>* tryGetVoiceToStop(const Steinberg::Vst::NoteOffEvent& event)
        {
            for (auto& voice : voices)
            {
