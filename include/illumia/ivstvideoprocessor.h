...
public:
  virtual Steinberg::tresult PLUGIN_API setBusArrangements(
      const struct Graphics &graphics, VideoBusArrangement *inputs,
      Steinberg::int32 numIns, VideoBusArrangement *outputs,
      Steinberg::int32 numOuts) = 0;
  virtual Steinberg::tresult PLUGIN_API
  getBusArrangement(Steinberg::Vst::BusDirection dir, Steinberg::int32 index,
                    VideoBusArrangement &bus) = 0;
  virtual Steinberg::tresult PLUGIN_API process(const struct Graphics &graphics,
                                                VideoData &videoData) = 0;

  static const Steinberg::FUID iid;
};

DECLARE_CLASS_IID(IVideoProcessor, 0xC729CEAF, 0x2DC44D9A, 0xB65D2209,
                  0xBBF0863E)
} // namespace illumia
