...
concept DerivedFromEffect =
    std::derived_from<Object, Steinberg::Vst::AudioEffect> ||
    std::derived_from<Object, illumia::VideoEffect>;

template <typename Object>
concept ValidObject = DerivedFromEffect<Object> &&
  requires()
{
...
struct Module final : Steinberg::IPluginFactory2 {
  static_assert(sizeof...(Objects) <= ILLUMIA_GENERATED_GUID_COUNT,
                "You need more GUIDS!"
                "Increase the GENERATED_GUID_COUNT option when adding this "
                "module in CMake.");
  ...
  getFactoryInfo(Steinberg::PFactoryInfo *info) override {
    *info = Steinberg::PFactoryInfo{ILLUMIA_MODULE_VENDOR, ILLUMIA_MODULE_URL,
  ...
  Steinberg::tresult PLUGIN_API
  getClassInfo(int32_t index, Steinberg::PClassInfo *info) override {
    return getClassInfo(index, info,
                        std::make_index_sequence<sizeof...(Objects)>{});
  }
  template <size_t... Is>
  Steinberg::tresult getClassInfo(int32_t index, Steinberg::PClassInfo *info,
                                  std::index_sequence<Is...>) {
    return (getClassInfo<Is, Objects>(index, info) || ...)
               ? Steinberg::kResultOk
  ...
  bool getClassInfo(int32_t index, Steinberg::PClassInfo *info) {
    if (Index != index)
      return false;
    *info = Steinberg::PClassInfo{classGuids[Index],
                                  Steinberg::PClassInfo::kManyInstances,
                                  Object::getCategory(), Object::getName()};
    return true;
  }
  Steinberg::tresult PLUGIN_API
  getClassInfo2(int32_t index, Steinberg::PClassInfo2 *info) override {
    return getClassInfo2(index, info,
                         std::make_index_sequence<sizeof...(Objects)>{});
  }
  template <size_t... Is>
  Steinberg::tresult getClassInfo2(int32_t index, Steinberg::PClassInfo2 *info,
                                   std::index_sequence<Is...>) {
    return (getClassInfo2<Is, Objects>(index, info) || ...)
               ? Steinberg::kResultOk
               : Steinberg::kInvalidArgument;
  }
  template <size_t Index, typename Object>
  bool getClassInfo2(int32_t index, Steinberg::PClassInfo2 *info) {
    if (Index != index)
      return false;
    *info = Steinberg::PClassInfo2{classGuids[Index],
                                   Steinberg::PClassInfo::kManyInstances,
                                   Object::getCategory(),
                                   Object::getName(),
                                   0,
  ...
  Steinberg::tresult PLUGIN_API createInstance(const char *cid, const char *iid,
                                               void **obj) override {
    return createInstance(cid, iid, obj,
                          std::make_index_sequence<sizeof...(Objects)>{});
  }
  template <size_t... Is>
  Steinberg::tresult createInstance(const char *cid, const char *iid,
                                    void **obj, std::index_sequence<Is...>) {
    Steinberg::tresult result{Steinberg::kNoInterface};
    if ((createInstance<Is, Objects>(cid, iid, obj, result) || ...))
      return result;
    *obj = nullptr;
    return result;
  }
  template <size_t Index, typename Object>
  bool createInstance(const char *cid, const char *iid, void **obj,
                      int32_t &result) {
    if (!Steinberg::FUnknownPrivate::iidEqual(classGuids[Index], cid))
      return false;
    auto instance{Steinberg::owned(new Object())};
    result = instance->queryInterface(iid, obj);
    return true;
  }
  Steinberg::tresult PLUGIN_API queryInterface(const char iid[16],
                                               void **obj) override {
