...
struct FaustWrapperBase {
  ...
  void closeBox() {}
  void addVerticalSlider(const char *name, float *ptr, float init, float min,
...
template <typename T> struct FaustWrapper : FaustWrapperBase {
  void init(int sampleRate, int maxBatchSize,
            std::vector<NoteCV> noteCVs = {}) {
    FaustWrapperBase::init(maxBatchSize, std::move(noteCVs));
    generatedDsp.init(sampleRate);
    generatedDsp.buildUserInterface(this);
  }
  void process(int numFrames, float **inputs, float **outputs) {
