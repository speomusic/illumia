...
inline MTLPixelFormat toMTLPixelFormat(TextureBusType TextureBusType) {
  ...
  default:
    throw std::runtime_error{"Unsupported TextureBusType"};
  }
}
#endif
...
struct Texture {
  Texture(const TextureBusArrangement &arrangement, id<MTLDevice> device) {
    auto descriptor = [MTLTextureDescriptor
        texture2DDescriptorWithPixelFormat:toMTLPixelFormat(arrangement.type)
  ...
  Texture(const Texture &other) {
    auto descriptor = [MTLTextureDescriptor
        texture2DDescriptorWithPixelFormat:other.mtlTexture.pixelFormat
